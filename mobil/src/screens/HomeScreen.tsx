...
export default function HomeScreen() {
  ...
  const [theme, setTheme] = useState<
    "friendship" | "courage" | "sharing" | "emotions"
  >("friendship");
  ...
  const generationHint =
    getGenerationStatusMessage(result, loading) ?? generationStatus;
  ...
  useEffect(() => {
    const fetchSubscription = async () => {
      ...
      try {
        const nextSubscription = await fetchCurrentSubscription<Subscription>(
  ...
  const handleGenerate = async () => {
    if (!user || !session) {
      ...
      setError(
        "Masal üretmek için giriş yapmalısınız. Hazır olduğunuzda giriş ekranına geçebilirsiniz.",
      );
      return;
    }
      ...
      void saveStoryToHistory(user.id, {
        ...
        .then((savedEntry) => {
          if (!savedEntry.audioBase64 && data.audioBase64) {
            console.warn(
              "Story history audio omitted to keep local storage lighter.",
            );
          }
        })
        .catch((historyError) => {
    ...
    <GradientBackground>
      ...
      >
        ...
        <View style={styles.navBar}>
          ...
          <View style={styles.navActions}>
            ...
            <TouchableOpacity
              ...
              accessibilityLabel={
                mode === "light" ? "Karanlık temaya geç" : "Aydınlık temaya geç"
              }
            >
        ...
        <View style={styles.header}>
          ...
          >
            <Text style={styles.badgeText}>
              ⭐ Yapay zeka destekli, çocuklara özel sesli masallar.
            </Text>
          </LinearGradient>
          <Text style={[styles.headerTrustCopy, { color: colors.textMuted }]}>
            Yaşa uygun dil, reklamsız deneyim ve hızlı sesli anlatım tek akışta
            hazırlanır.
          </Text>
        </View>
        ...
        <GlassCard style={styles.inputCard}>
            ...
            {EXAMPLE_TOPICS.map((example) => (
              <TouchableOpacity
                ...
                style={[
                  ...
                  {
                    backgroundColor:
                      topic === example ? Colors.primary[500] : colors.surface,
                    borderColor:
                      topic === example
                        ? Colors.primary[500]
                        : colors.inputBorder,
              ...
              >
                <Text
                  style={[
                    ...
                    {
                      color:
                        topic === example ? Colors.white : colors.textSecondary,
                    },
                  ]}
                >
                  {example}
              ...
              <View style={styles.toggleRow}>
                {(["short", "medium"] as const).map((val) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.toggleBtn,
                      length === val && styles.toggleBtnActive,
                    ]}
                  >
                    <Text
                      style={[
                        styles.toggleText,
                        length === val && styles.toggleTextActive,
                      ]}
                    >
                      {val === "short" ? "📖 Kısa" : "📚 Orta"}
                    </Text>
                    <Text
                      style={[
                        styles.toggleSub,
                        length === val && styles.toggleSubActive,
                      ]}
                    >
              ...
              <View style={styles.toggleRow}>
                {(["3-5", "6-8"] as const).map((val) => (
                  <TouchableOpacity
                    ...
                    style={[
                      styles.toggleBtn,
                      ageRange === val && styles.toggleBtnActiveOrange,
                    ]}
                  >
                    <Text
                      style={[
                        styles.toggleText,
                        ageRange === val && styles.toggleTextActive,
                      ]}
                    >
          ...
          >
            <Text
              style={[styles.advancedText, { color: colors.textSecondary }]}
            >
          ...
          {user && subscription ? (
            <View style={styles.creditHintBox}>
              <Text style={[styles.creditHintTitle, { color: colors.text }]}>
                Bu işlem 1 kredi kullanır
              </Text>
              <Text
                style={[styles.creditHintText, { color: colors.textSecondary }]}
              >
                Şu an hesabınızda {creditsRemaining} kredi var. Masal
                oluşturulduktan sonra kalan kredi bilginiz otomatik
                güncellenecek.
              </Text>
              {creditsRemaining <= 0 ? (
                <Text style={styles.creditWarningText}>
                  Yeni masal oluşturmak için önce paketinizi yükseltmeniz
                  gerekiyor.
                </Text>
              ) : null}
            </View>
          ...
          {!user && willUseCredit ? (
            <Text style={[styles.preAuthHint, { color: colors.textMuted }]}>
              Masal üretmek için giriş yaptığınızda kredi kullanımı ve kalan
              hakkınız burada görünür.
            </Text>
          ) : null}
        ...
        <View style={styles.footer}>
          <View style={styles.footerRow}>
            <Text style={[styles.footerText, { color: colors.textMuted }]}>
              🛡 Çocuk güvenliği önceliğimiz
            </Text>
            <Text style={[styles.footerText, { color: colors.textMuted }]}>
              🔒 Verileriniz saklanmaz
            </Text>
            <Text style={[styles.footerText, { color: colors.textMuted }]}>
              💰 Her gün ücretsiz 1 hikaye
            </Text>
          </View>
