...
export default function DashboardPage() {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Fetch subscription
      const subResponse = await fetch('/api/subscription/current', { headers });
      ...
      if (subResponse.status === 404) {
        ...
        const retryResponse = await fetch('/api/subscription/current', { headers });
        if (retryResponse.ok) {
      ...
      // Fetch transactions
      const transResponse = await fetch('/api/subscription/transactions', { headers });
      if (transResponse.ok) {
  ...
  const getPlanColor = (plan: string) => {
    switch (plan) {
      case 'free': return 'bg-neutral-500';
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          ...
          <div className="glass-card p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className={`px-3 py-1 rounded-full text-xs font-bold text-white ${subscription ? getPlanColor(subscription.plan) : 'bg-neutral-500'}`}>
                {subscription ? getPlanName(subscription.plan) : 'Yükleniyor...'}
              </div>
            </div>
            <Link href="/pricing" className="inline-flex items-center text-primary-500 hover:text-primary-600 font-semibold text-sm">
              Paketi Yükselt →
            </Link>
          </div>
          ...
          <div className="glass-card p-6">
            ...
            <div className="flex items-center gap-2 mb-2">
              <div className={`w-3 h-3 rounded-full ${subscription?.status === 'active' ? 'bg-green-500 animate-pulse' : 'bg-red-500'}`}></div>
        ...
        <div className="glass-card p-6 mb-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <Link href="/" className="flex items-center gap-3 p-4 rounded-xl bg-gradient-to-br from-primary-50 to-warm-50 dark:from-primary-900/20 dark:to-warm-900/20 hover:scale-105 transition-all">
              <div className="w-12 h-12 rounded-full bg-primary-500 flex items-center justify-center text-white text-2xl">
                ✨
              </div>
              <div>
                <p className="font-semibold text-neutral-800 dark:text-neutral-100">Masal Oluştur</p>
                <p className="text-xs text-neutral-600 dark:text-neutral-400">Yeni bir hikaye yarat</p>
              </div>
            </Link>

            <Link href="/pricing" className="flex items-center gap-3 p-4 rounded-xl bg-gradient-to-br from-secondary-50 to-purple-50 dark:from-secondary-900/20 dark:to-purple-900/20 hover:scale-105 transition-all">
              <div className="w-12 h-12 rounded-full bg-secondary-500 flex items-center justify-center text-white text-2xl">
                💎
              </div>
              <div>
                <p className="font-semibold text-neutral-800 dark:text-neutral-100">Paket Yükselt</p>
                <p className="text-xs text-neutral-600 dark:text-neutral-400">Daha fazla masal</p>
              </div>
            </Link>
            ...
            <button className="flex items-center gap-3 p-4 rounded-xl bg-gradient-to-br from-warm-50 to-yellow-50 dark:from-warm-900/20 dark:to-yellow-900/20 hover:scale-105 transition-all">
              ...
              <div>
                <p className="font-semibold text-neutral-800 dark:text-neutral-100">Destek</p>
                <p className="text-xs text-neutral-600 dark:text-neutral-400">Yardıma mı ihtiyacınız var?</p>
              </div>
            </button>
          </div>
        </div>
        ...
        <div className="glass-card p-6">
          ...
          {transactions.length === 0 ? (
            <div className="text-center py-8">
              <p className="text-neutral-500 dark:text-neutral-500">Henüz işlem geçmişiniz bulunmuyor.</p>
            </div>
          ) : (
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-neutral-200 dark:border-neutral-700">
                    <th className="text-left py-3 px-4 text-sm font-semibold text-neutral-600 dark:text-neutral-400">Tarih</th>
                    <th className="text-left py-3 px-4 text-sm font-semibold text-neutral-600 dark:text-neutral-400">Açıklama</th>
                    <th className="text-left py-3 px-4 text-sm font-semibold text-neutral-600 dark:text-neutral-400">Tip</th>
                    <th className="text-right py-3 px-4 text-sm font-semibold text-neutral-600 dark:text-neutral-400">Miktar</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    <tr key={transaction.id} className="border-b border-neutral-100 dark:border-neutral-800 hover:bg-neutral-50 dark:hover:bg-neutral-800/50">
                      ...
                      <td className="py-3 px-4">
                        <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
                          transaction.type === 'purchase' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' :
                          transaction.type === 'usage' ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400' :
                          transaction.type === 'refund' ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400' :
                          'bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-400'
                        }`}>
                          {transaction.type === 'purchase' ? 'Satın Alma' :
                           transaction.type === 'usage' ? 'Kullanım' :
                           transaction.type === 'refund' ? 'İade' : 'Bonus'}
                        </span>
                      </td>
